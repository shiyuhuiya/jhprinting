    ...
    <div class="address">
      ...
      <div class="info" v-if="selectedAddress.address_id">
        ...
        <div class="info-address">
          {{ Object.values(selectedAddress.region).join(' ')+selectedAddress.detail }}
        </div>
      </div>
      ...
      <div class="right-icon" @click="selectAddress">
        <van-icon name="arrow" />
      </div>
    </div>
...
export default {
  ...
  data () {
    return {
      addressList:[],//地址列表
      personal:{},//用户信息，包括用户余额
      order:{},//订单信息
      remark: '',//用户写的备注
      index:this.$store.state.index//用户选中的地址
    }
  },
  created(){
    //获取用户所有的的地址信息
  ...
  methods: {
    ...
    async getOrderList(){
      ...
      if(this.mode==='cart'){
        ...
        res = await checkOrder(this.mode,{goodsId:this.goodsId,goodsSkuId:this.goodsSkuId,goodsNum:this.goodsNum})
      }
    ...
    async submit(){
      let res2 = {}
      if(this.mode==='cart'){
        res2 = await submitOrder(this.mode,{cartIds:this.cartIds,remark: this.remark})
      }else{
        res2 = await submitOrder(this.mode,{goodsId:this.goodsId,goodsSkuId:this.goodsSkuId,goodsNum:this.goodsNum,remark: this.remark})
      }
      // console.log(res2)
